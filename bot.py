...
async def debug_log_message(client, message: Message):
    print(f"[debug] message from user_id={message.from_user.id if message.from_user else '?'} text={message.text or message.caption or '<media>'}")


@app.on_callback_query(group=10)
async def debug_log_callback(client, callback_query: CallbackQuery):
    print(f"[debug] callback from user_id={callback_query.from_user.id} data={callback_query.data}")
...
async def auto_delete(client, chat_id, message_ids, minutes):
    print(f"[auto-delete] scheduled: chat={chat_id} in {minutes} min for {len(message_ids)} messages")
    await asyncio.sleep(minutes * 60)
    try:
        await client.delete_messages(chat_id, message_ids)
        print(f"[auto-delete] deleted {len(message_ids)} messages in chat {chat_id}")
    except Exception as e:
